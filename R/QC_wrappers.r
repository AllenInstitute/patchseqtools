...
calculatePatchSeqQCMetrics2 <- function(pat_df, facs_df,
  ...
  marker_sums = lapply(dataset_cell_types, function(cell_type) {
    ...
    # curr_marker_type = paste0(curr_marker_type,
    # '_on')
    ...
    mks <- markers[c(curr_marker_type, compare_cell_types)] %>%
      unlist %>% unique
    compare_expr_profile = facs_df[facs_df$major_type ==
      cell_type, mks] %>% log2 %>% colMeans
    ...
    compare_cell_type_exprs = aibs_med_exprs[compare_cell_types,
      compare_cell_types] %>% as.matrix %>%
      diag
    normalizedContamValues = apply(contam_values[,
      compare_cell_types], 1, function(x) (x -
      expected_expr)/(compare_cell_type_exprs -
      expected_expr))

    contam_sum = normalizedContamValues %>% repWZero() %>%
      colSums

    marker_sum_norm = normalizeContam(contam_values,
      aibs_med_exprs, c(curr_marker_type, compare_cell_types))
    marker_sum_norm_vec = contam_values[, curr_marker_type]/aibs_med_exprs[curr_marker_type,
      curr_marker_type]


    quality_score = rbind(compare_expr_profile %>%
      t() %>% as.data.frame, df[, mks] %>% log2) %>%
      t() %>% cor(method = "spearman")
    quality_score = quality_score[1, -1]

    out_df = data.frame(sample_id = rownames(df),
      marker_sum = marker_expr, marker_sum_norm = marker_sum_norm_vec,
      contam_sum = contam_sum, quality_score = quality_score)
    out_df = cbind(out_df, marker_sum_norm %>%
      t())
    return(out_df)
  })
  ...
  marker_sums = merge(pat_df %>% dplyr::select(dplyr::one_of("sample_id",
    "major_type", "contam_type")), marker_sums,
    by = "sample_id")
